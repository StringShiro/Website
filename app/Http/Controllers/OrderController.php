...
use Illuminate\Support\Facades\Facade\App;
...
class OrderController extends Controller
{
	...
	public function print_order_convert($checkout_code){
		...
		<center><h2>3 Sir Company</h2></center>
		<p>Người đặt hàng</p>
    ...
    public function manage_order(){
    	$order = Order::orderby('order_id','DESC')->paginate(6);
    	return view('admin.manage_order')->with(compact('order'));
    }

	public function history(Request $request) {
		if(!Session::get('customer_id')){
			return redirect('dang-nhap')->with('error', 'Hãy đăng nhập để xem lịch sử mua hàng!');
		}
		else {
    		//slide
			$slider = Slider::orderBy('slider_id','DESC')->where('slider_status','1')->take(4)->get();
			//seo 
			$meta_desc = "lich-su-don-hang"; 
			$meta_keywords = "lich su don hang";
			...
			->with('slider',$slider)->with('order',$order); //1
